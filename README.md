...
**What this project builds:** a leakage-safe weekly merchant panel, a
discrete-time hazard model, isotonic calibration, a FAR-controlled
alert policy, a historical decision replay, and an economic sensitivity
analysis, on the Olist Brazilian marketplace dataset.

**What it found:** a naive read of the model produced 0.89–0.97 AUC —
an apparent strong early-warning signal — but auditing that number
against each seller's actual last order collapsed it to 0.53–0.59 AUC:
no reliable multi-week advance warning before a seller's last order.
What the model can do instead is flag a minority of already-quiet
sellers earlier than the fixed N=8 silence rule — 36% of cessations, at
a median two-week acceleration — treated here as a modest
inactivity-detection result, not distress prediction.

![The merchant view in the interactive demo: hazard trajectory, flag status, cost trade-off, and simulated policy action for a single seller](figures/readme_demo_screenshot.png)
...
sides in R$ per 1,000 merchant-weeks, on the calibrated model: **under
the assumed cost model, the simulated model-triggered policy has lower
estimated cost than the N=8 rule at every tested operating point**,
...
<details>
<summary>Mechanism, and the practical reading for deployment</summary>

Mechanism, quantified rather than hand-waved: the row-level
...
<details>
<summary>The earlier wrong framing of this check, and its limits</summary>

The earlier framing of this check — that train-derived
...
<details>
<summary>Mechanism: why a monotonic recalibration wasn't quite a no-op</summary>

Mechanism: the FAR sweep was never actually
